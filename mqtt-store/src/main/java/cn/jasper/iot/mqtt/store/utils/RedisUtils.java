...
public class RedisUtils {
    ...
    public Long expire(String key, int value){
        return expire(key,value,database);
    }
...
//    }
     ...
     * @return
     */
    public Map<String,Map<String,String>> scan(String pre,int indexdb){
        if(pre==null){
            pre = "";
        }
        Map<String, Map<String, String>> map = new HashMap<>();
        ScanParams match = new ScanParams().match(pre + "*");
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            jedis.select(indexdb);
            ...
            do {
                scan = jedis.scan(scan == null ? ScanParams.SCAN_POINTER_START : scan.getCursor(), match);
                for (String key : scan.getResult()) {
                    ConcurrentHashMap<String, String> map1 = new ConcurrentHashMap<>();
                    Map<String, String> map2 = this.hgetall(key);
                    if (map2 != null && !map2.isEmpty()) {
                        map2.forEach((k, v) -> {
                            map1.put(k, v);
                        });
                        map.put(key.substring(pre.length()), map1);
                    }
                }
    ...
    public Map<String,Map<String,String>> scan(String pre){
        return scan(pre,database);
    }
